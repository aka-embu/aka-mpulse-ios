...
@property (class, nonatomic, assign) BOOL configured;

/**
 * @deprecated Omit this init call and set API key in Info.plist.
 * com.akamai : {
 *    mpulse : {
 *        api_key = ABCDE12345
 *    }
 ...
 */
+(MPulse *) initializeWithAPIKey:(NSString *)APIKey DEPRECATED_MSG_ATTRIBUTE("Init no longer needed. Set API key in Info.plist.");
 ...
 * @return mPulse instance
 */
+(MPulse *) initializeWithAPIKey:(NSString *)APIKey withSettings:(MPulseSettings *)settings DEPRECATED_MSG_ATTRIBUTE("Init no longer needed. Set API key in Info.plist.");;
